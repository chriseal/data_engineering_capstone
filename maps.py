...
# SMALL-SIZE MAPS for SAS and other datasets
...
SAS_PORT_TO_CITY = dict([(k, ' '.join([c.capitalize() for c in v.split(',')[0].split(' ')]).split(' - ')[0].strip() ) for k, v in SAS_PORT_TO_RAW.items()])
clean_port_p = re.compile(r"([nN]o\s+[Pp]ort\s+[Cc]ode)(\s?\(.+\))")
SAS_PORT_TO_CITY = dict([(k, re.sub(clean_port_p, r"\1", v if 'no port code' in v.lower() in v else v)) for k, v in SAS_PORT_TO_CITY.items()])
SAS_PORT_TO_CITY = dict([(k, "Collapsed" if v.lower().startswith('collapsed') else v) for k, v in SAS_PORT_TO_CITY.items()])
get_state_p = re.compile(r"^(\s*)([A-Z]+)(\s?)(.*)$")
SAS_PORT_TO_STATE = dict([(k, re.sub(get_state_p, r"\2", v.split(', ')[-1]) if ', ' in v else None) for k, v in SAS_PORT_TO_RAW.items()])